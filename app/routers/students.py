...
#chat gpt code to fix app deployment crash
from fastapi import APIRouter, Depends, HTTPException, UploadFile, File, Form, status
from sqlalchemy.orm import Session
from typing import List
import json
...
def safe_face_encoding(image_url: str):
    """
    Wrapper to prevent deployment crash if ML libs fail.
    Keeps behavior same in local + safe in cloud.
    """
    try:
        return face_service.extract_face_encoding(image_url)
    except Exception as e:
        # log but DO NOT crash app
        import logging
        logging.error(f"Face encoding failed: {str(e)}")
        return None
...
async def register_student(
    ...
    face_encoding = safe_face_encoding(image_url)

    if face_encoding is None:
        raise HTTPException(
            status_code=400,
            detail="No face detected in image OR face service unavailable"
        )
...
async def update_student_photo(
    ...
    if face_encoding is None:
        raise HTTPException(
            status_code=400,
            detail="Face detection failed or service unavailable"
        )
